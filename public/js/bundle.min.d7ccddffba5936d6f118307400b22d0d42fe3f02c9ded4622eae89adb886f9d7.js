...
oldIdx=idx}});},false);;'use strict';class Termynal{constructor(container='#termynal',options={}){this.container=(typeof container==='string')?document.querySelector(container):container;this.pfx=`data-${options.prefix||'ty'}`;this.startDelay=options.startdelay||600;this.typeDelay=options.typedelay||90;this.lineDelay=options.linedelay||1000;this.progressLength=options.progresslength||32;this.progressChar=options.progresschar||'█';this.progressPercent=options.progressPercent||100;this.showPercent=("showpercent"in options&&options.showpercent)||(!"showpercent"in options&&true);this.onExitCommand=options.onExitCommand;this.cursor=options.cursor||'▋';if(!options.noInit)this.init()}
...
var terms={};var termOptions={};function open(modalElement){modal=modalElement
...
request.open('POST','https://usebasin.com/f/b097ffcf700d',true)
